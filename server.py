...
def showSummary():
    ...
    if list_clubs:
        return render_template('welcome.html', club=list_clubs[0], competitions=competitions)
...
def purchasePlaces():
    ...
    competitionDate = datetime.strptime(competition.get('date', None), "%Y-%m-%d %H:%M:%S")
    ...
    for compet in competitions:
        if compet['name'] == competition['name']:
            compet['numberOfPlaces'] = int(compet['numberOfPlaces']) - placesRequired
    competitions_dict = {
